...
if (!executablePath) throw new Error("Firefox not found; set FIREFOX_PATH");

const url = process.argv[2] ?? "https://www.reddit.com/r/popular/?geo_filter=IN";
const browser = await puppeteer.launch({ browser: "firefox", executablePath, headless: !process.env.HEADFUL, args: ["-no-remote"] });
let failed = false;
const check = (ok, msg) => {
...
try {
  await browser.installExtension(new URL("../dist", import.meta.url).pathname);
